(module coin-faucet 'faucet-admin-keyset
  ...
  (deftable history1-table: {history})
  ...
  (defun request-coin:string (address:string amount:decimal)
    ...
    (with-default-read history1-table address
  ...
  (defun read-history:object{history} (address:string)
    ...
    (read history1-table address))

  (defun curr-time ()
    (at 'block-time (chain-data)))

  (defun read-insert (key:string)
   (with-read history-table key {
    "total-coins-earned" := total-coins-earned,
    "total-coins-returned" := total-coins-returned
    }
     (insert history1-table key {
       "total-coins-earned" : total-coins-earned,
       "total-coins-returned" : total-coins-returned,
       "last-request-time": EPOCH
     })))
  ...
  (defun migrate ()
    (map (read-insert) (keys history-table)))

)

(create-table history1-table)
(migrate)
